...
class BeatVisitor(llullVisitor):
    ...
    def visitExpr(self, ctx):
        l = list(ctx.getChildren())
        if len(l) == 1:
            if hasattr(l[0], 'getRuleIndex') and llullParser.ruleNames[l[0].getRuleIndex()] == 'get_array':
        ...
        return str(self.visit(l[0])) + ' ' + l[1].getText() + ' ' + str(self.visit(l[2]))
    ...
    def visitIf_else (self, ctx):
        ...
        ret = ""
        while (i < len(l)):
            if (i == 0): retLoop = l[i].getText() 
            else: retLoop = l[i].getText() 
    ...
    def visitAccio (self, ctx):
        l = list(ctx.getChildren())
        self.spaces += 1
        ret = colored (l[0].getText(), 'red', attrs = ['bold']) + ' ' + colored (l[1].getText(), 'magenta', attrs = ['bold']) + '('
        i = 3
        while i < len(l) and l[i].getText() != ')':
            ret += l[i].getText()
            if (l[i].getText() == ','): ret += ' '
    ...
    def visitInvocacio(self, ctx):
        ...
        while i < len(l):
            if hasattr(l[i], 'getRuleIndex') and llullParser.ruleNames[l[i].getRuleIndex()] == 'expr':
                ret += self.visit(l[i])
            else:
                ret += l[i].getText()
                if (l[i].getText() == ','): ret += ' '
